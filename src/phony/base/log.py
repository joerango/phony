...
class NamedLogger(object):
    ...
    def _variable(self, variable, value, label='', with_arguments=True, width=MAXIMUM_TRACE_WIDTH,
    ...
    def _calling_instance_method_name_and_args(frame_level=0):
        ...
        caller_args = [values[arg] for arg in args]
...
def pretty_args(args, limit):
    ...
    val = ', '.join([_f for _f in map(stringify, args) if _f])
    if len(val) > limit:
        return val[:limit] + '...'
    else:
        return val
